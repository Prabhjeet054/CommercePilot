/**
 * Phase 19 — Complete purchase flow integration (PRD §33 demos + daily-limit edge).
 *
 * Checkout uses the same test-double as Phase 16 (placeholder Razorpay keys).
 * After verify, the suite delivers a signed payment.captured webhook (Phase 18)
 * so OrderSuccess only appears once the backend reports COMPLETED.
 */
...
function uniqueEmail(): string {
  return `phase19-${Date.now()}-${Math.random().toString(36).slice(2, 10)}@e2e.commercepilot.test`;
}
...
async function registerCustomer(page: Page): Promise<{ accessToken: string; email: string }> {
  ...
  await page.getByLabel("Name").fill("Phase 19 Tester");
...
async function deliverPaymentCapturedWebhook(input: {
  ...
  const response = await fetch(`${apiBase()}/webhooks/razorpay`, {
    ...
    headers: {
      ...
      "x-razorpay-event-id": `evt_phase19_${randomUUID()}`,
    },
    body: raw,
  });
...
async function payThroughCheckout(page: Page): Promise<{
  ...
  // Must not show Order Success before webhook / COMPLETED.
...
test("Scenario 1: shoe → ALLOW → Checkout → verify → webhook → COMPLETED → Order Success", async ({
  page,
}) => {
...
test("Scenario 2: laptop → approve → Checkout → webhook → COMPLETED", async ({ page }) => {
  await installCheckoutDouble(page);
  await registerCustomer(page);
  await saveDemoPolicy(page);
...
test("Scenario 3: blocked category → DENY with zero Razorpay create-order calls", async ({
  page,
}) => {
...
test("Scenario 4: shoe COMPLETED then second shoe hits DAILY_LIMIT_EXCEEDED", async ({ page }) => {
  await installCheckoutDouble(page);
  await registerCustomer(page);
